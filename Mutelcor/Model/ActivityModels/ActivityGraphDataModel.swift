...
public class ActivityGraphData {
	...
	required public init?(json: JSON) {
        ...
        guard let date = Date.dateFormatter.date(from: json["activity_date"].stringValue) else {
            return nil
        }
	...
	func jsonRepresentation() -> JSONDictionary {
        ...
        Date.dateFormatter.dateFormat = DateFormat.yyyyMMdd.rawValue
		dictionary["activity_date"] = Date.dateFormatter.string(from: self.activityDate)
