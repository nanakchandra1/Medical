...
- (void)drawNeedle
{
    switch (_needleStyle)
    {
        case WMGaugeViewNeedleStyle3D:
        {
            ...
            [needlePath addArcWithCenter:CGPointMake(FULL_SCALE(center.x , center.y)) radius:(_needleWidth*self.bounds.size.width) startAngle:(22/7) endAngle:0 clockwise:false];
            [needlePath closePath];
            
            needleLayer.path = needlePath.CGPath;
            needleLayer.backgroundColor = [[UIColor clearColor] CGColor];
            needleLayer.fillColor = CGRGB(50, 50, 50);
...
- (void)addTextLayer
{
    CAShapeLayer *textContainerLayer = [CAShapeLayer layer];
    UIBezierPath *textContainerPath = [UIBezierPath bezierPath];
    
    CGFloat width = 0.07;
    CGFloat height = 0.115;
    
    CGFloat radius = 0.010;
    CGFloat arrowRadius = 0.008;
    
    CGFloat arrowWidth = 0.030;
    CGFloat arrowHeight = 0.015;
    
    CGPoint startingPoint = CGPointMake(FULL_SCALE(center.x - width + radius, center.y - _needleHeight));
    CGPoint upperRightCenter = CGPointMake(FULL_SCALE(center.x + width - radius, center.y - _needleHeight + radius));
    CGPoint bottomRightCenter = CGPointMake(FULL_SCALE(center.x + width - radius, center.y - _needleHeight + height - radius - arrowHeight));
    CGPoint arrowRight = CGPointMake(FULL_SCALE(center.x + arrowWidth/2 + arrowRadius, center.y - _needleHeight + height + arrowRadius - arrowHeight));
    CGPoint arrowBottom = CGPointMake(FULL_SCALE(center.x, center.y - _needleHeight + height - arrowRadius));
    CGPoint arrowLeft = CGPointMake(FULL_SCALE(center.x - arrowWidth/2 - arrowRadius, center.y - _needleHeight + height + arrowRadius - arrowHeight));
    CGPoint bottomLeftCenter = CGPointMake(FULL_SCALE(center.x - width + radius, center.y - _needleHeight + height - radius - arrowHeight));
    CGPoint upperLeftCenter = CGPointMake(FULL_SCALE(center.x - width + radius, center.y - _needleHeight + radius));
    
    [textContainerPath moveToPoint:startingPoint];
    [textContainerPath addArcWithCenter:upperRightCenter radius:(radius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(270) endAngle:DEGREES_TO_RADIANS(0) clockwise:YES];
    [textContainerPath addArcWithCenter:bottomRightCenter radius:(radius*self.bounds.size.width) startAngle:0 endAngle:DEGREES_TO_RADIANS(90) clockwise:YES];
    [textContainerPath addArcWithCenter:arrowRight radius:(arrowRadius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(270) endAngle:DEGREES_TO_RADIANS(225) clockwise:NO];
    [textContainerPath addArcWithCenter:arrowBottom radius:(arrowRadius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(45) endAngle:DEGREES_TO_RADIANS(135) clockwise:YES];
    [textContainerPath addArcWithCenter:arrowLeft radius:(arrowRadius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(315) endAngle:DEGREES_TO_RADIANS(270) clockwise:NO];
    [textContainerPath addArcWithCenter:bottomLeftCenter radius:(radius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(90) endAngle:DEGREES_TO_RADIANS(180) clockwise:YES];
    [textContainerPath addArcWithCenter:upperLeftCenter radius:(radius*self.bounds.size.width) startAngle:DEGREES_TO_RADIANS(180) endAngle:DEGREES_TO_RADIANS(270) clockwise:YES];
    [textContainerPath closePath];
    
    textContainerLayer.path = textContainerPath.CGPath;
    textContainerLayer.backgroundColor = [[UIColor clearColor] CGColor];
    textContainerLayer.fillColor = [[[UIColor whiteColor] colorWithAlphaComponent:0.9] CGColor];
    textContainerLayer.strokeColor = CGRGB(210,210,210);
    textContainerLayer.lineWidth = 0.5;
    
    needleTextlayer = [CATextLayer layer];
    CGRect containerRect = CGPathGetPathBoundingBox(textContainerPath.CGPath);
    needleTextlayer.bounds = CGRectMake(0, 0, containerRect.size.width, containerRect.size.height);
    needleTextlayer.position = CGPointMake(CGRectGetMidX(containerRect), CGRectGetMidY(containerRect)+3);
    [self updateNeedleText:_value];
    needleTextlayer.foregroundColor = [[UIColor blackColor] CGColor];
    //textLayer.font = (__bridge CFTypeRef _Nullable)([UIFont systemFontOfSize:8]);
    needleTextlayer.fontSize = 12;
    needleTextlayer.alignmentMode = kCAAlignmentCenter;
    needleTextlayer.contentsScale = [[UIScreen mainScreen] scale];
    needleTextlayer.rasterizationScale = [[UIScreen mainScreen] scale];
    [needleTextlayer setWrapped:(YES)];
    [textContainerLayer addSublayer:needleTextlayer];
    
    // Text Layer shadow
    needleTextlayer.shadowColor = CGRGBA(69, 69, 69, 0.3);
    needleTextlayer.shadowRadius = 0.5;
    
    [rootNeedleLayer addSublayer:textContainerLayer];
}

- (void)updateNeedleText:(float)value
{
    NSString *valueStr = [NSString stringWithFormat:@"%i", (int)value];
    [needleTextlayer setString:valueStr];
}
