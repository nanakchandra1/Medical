...
public class RTCClient: NSObject {
    ...
    fileprivate var isSpeakerEnabled = false
    ...
    public func muteAudio() {
        peerConnection?.localStreams.first?.audioTracks.first?.isEnabled = false
    }
    ...
    public func muteVideo() {
        peerConnection?.localStreams.first?.videoTracks.first?.isEnabled = false
    }
    ...
    public func switchCamera() {
        guard let videoSource = peerConnection?.localStreams.first?.videoTracks.first?.source as? RTCAVFoundationVideoSource else {
            return
        }
        if videoSource.canUseBackCamera {
            videoSource.useBackCamera = !videoSource.useBackCamera
        } else {
            print("Cannot use back camera")
        }
    }
